...
function getImageWC() {
  ...
  case ${_mode} in
    ...
    rows) 
      ...
      convert ${_image} -resize "1x${_tot}!" ${_tmpFile}
      ;;
    # Colapse to one row
    *)    
      _tot=$(identify -format "%w" ${_image}) 
      convert ${_image} -resize "${_tot}x1!" ${_tmpFile}
      ;;
  esac
  trace "  _tot : ${_tot}"
...
function removeMargins() {
  ...
  convert $_image -crop +${_left}+${_top} -crop -${_right}-${_bottom} $_dstImage
}
...
function splitImg() {
  ...
  local _imgName="$dstDir/$(basename $_image|sed -e 's/\..*//')"
  ...
  do
    ...
    then
      ...
      case $_mode in
        rows) convert $_myImg -crop x${#_frg}+0+0 $_file ;;
        *)    convert $_myImg -crop ${#_frg}x+0+0 $_file ;;
      esac
      _index=$(($_index+1)) 
    fi
    ...
    case $_mode in
      rows) convert $_myImg -chop 0x${#_frg}+0+0 $_myImg ;;
      *)    convert $_myImg -chop ${#_frg}x0+0+0 $_myImg ;;
    esac
  done
}
